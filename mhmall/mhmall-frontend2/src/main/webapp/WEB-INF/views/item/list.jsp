		...
		<c:forEach items="${itemList}" var="idata">
          <div class="col-xl-3 col-lg-6 col-md-6 mb-4">
            <div class="card h-100">
              <a style="display:block;overflow:hidden;" href="${pageContext.servletContext.contextPath}/item/view/${idata.no}/${kwdPath}">
              ...
              <div>
                <h5 class="text-center mr-2 ml-2" style="height:20px;line-height:20px;overflow:hidden;">
                  <a href="${pageContext.servletContext.contextPath}/item/view/${idata.no}/${kwdPath}" style="text-decoration:none;color:#000000;font-size:14px;">${idata.name}</a>
                </h5>
        ...
        <div class="row">
			...
			<ul class="pagination justify-content-center">
				...
				<c:if test="${(paging.rangeStart - 1) ge 1}">
					<li class="page-item">
						<a class="page-link" href="${pageContext.servletContext.contextPath}/item/list/${categoryNoPath}/${paging.rangeStart - 1}/${kwdPath}">◀</a>
					</li>
				</c:if>
				<c:forEach begin="${paging.rangeStart}" end="${paging.rangeStart + paging.pageCnt - 1}" var="i">
					...
					<c:if test="${i le paging.lastPage}">
						...
						<c:if test="${i ne paging.pages}">
							<li class="page-item"><a class="page-link" href="${pageContext.servletContext.contextPath}/item/list/${categoryNoPath}/${i}/${kwdPath}">${i}</a></li>
						</c:if>
					</c:if>
				</c:forEach>
				<!-- 최대 페이지보다 작거나 같으면 버튼활성화 -->
				<c:if test="${(paging.rangeStart + paging.pageCnt) le paging.lastPage}">
					<li class="page-item"><a class="page-link" href="${pageContext.servletContext.contextPath}/item/list/${categoryNoPath}/${paging.rangeStart + paging.pageCnt}/${kwdPath}">▶</a></li>
				</c:if>
				<!-- 최대 페이지보다 크면 버튼 비활성화 -->
